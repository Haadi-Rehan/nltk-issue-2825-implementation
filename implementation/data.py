...
# Note: expand_user_path() is defined later in this file, so we use os.path.expanduser directly here
...
def expand_user_path(path):
    ...
    :param path: Path string that may contain ~ or ~user
    :type path: str
    :return: Path with ~ expanded to home directory, or original path if expansion fails
    :rtype: str
    
    Example:
        >>> import os
        >>> result = expand_user_path("~/nltk_data")  # doctest: +SKIP
        >>> os.path.expanduser("~") in result  # doctest: +SKIP
        True
        >>> expand_user_path("/absolute/path")
        '/absolute/path'
        >>> expand_user_path(None) is None
        True
    
    .. note::
       This function is part of the implementation for GitHub Issue #2825,
       which adds tilde expansion support to NLTK path handling.
...
def find(resource_name, paths=None):
    ...
    # Expand ~ in all paths (Issue #2825: support tilde in paths)
